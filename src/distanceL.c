    ...
    for(j = 0 ; j < nc ; j++) {
        if(both_non_NA(x[i1], x[i2])) {
            if(!both_FINITE(x[i1], x[i2])) {
                ...
                Rprintf("Warning: treating non-finite values as NA");
            }
            else {
                if(x[i1] || x[i2]) {
                    count++;
...
void R_distance(double x[], int sel[], int *nr, int *nc, int *nsel,
                double *d, int *diag, int *method, double *p)
{
    int dc, i, j;
    ...
    else 
    {
        ...
        for (j = 0 ; j < *nr ; j++)
        {
            ...
            for (i = j ; i < *nr ; i++)
            {
                if (imap[j] != -1) 
                {
                    d[i+*nr*imap[j]] = (*method != MINKOWSKI) ?
                    distfun(x, *nr, *nc, i, j) : R_minkowski(x, *nr, *nc, i, j, *p);
                    if (imap[i] != -1) 
                    {
                        d[j+*nr*imap[i]] = d[i+*nr*imap[j]];
                    }
                }
                else 
                {
                    if (imap[i] != -1) 
                    {
                        d[j+*nr*imap[i]] = (*method != MINKOWSKI) ?
                        distfun(x, *nr, *nc, i, j) : R_minkowski(x, *nr, *nc, i, j, *p);
                    }
                }                
            }
...
SEXP CdistR(SEXP x, SEXP sel, SEXP smethod, SEXP p)
{
    SEXP ans;
    ...
    int N, diag = 0;
    double rp = asReal(p);
    int seli = asInteger(sel);
    if (seli==NA_INTEGER) 
    {
        nsel = NA_INTEGER;
        // allocate space for lower diag part without diagonal
        N = (double)nr * (nr-1)/2; /* avoid overflow for N ~ 50,000 */
    }
    else 
    {
        nsel = length(sel);
        // allocate space for full rectangular matrix
        N = (double)nr * nsel;
    }
    PROTECT(ans = allocVector(REALSXP, N));

    R_distance(REAL(x), INTEGER(sel), &nr, &nc, &nsel, REAL(ans), &diag, &method, &rp);
